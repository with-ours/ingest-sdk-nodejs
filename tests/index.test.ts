...
import { APIPromise } from '@oursprivacy/server-sdk/core/api-promise';

import util from 'node:util';
import OursPrivacy from '@oursprivacy/server-sdk';
import { APIUserAbortError } from '@oursprivacy/server-sdk';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['OURS_PRIVACY_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: OursPrivacy) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new OursPrivacy({ logger: logger, logLevel: 'debug' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new OursPrivacy({ logger: logger, logLevel: 'info' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['OURS_PRIVACY_LOG'] = 'debug';
      const client = new OursPrivacy({ logger: logger });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['OURS_PRIVACY_LOG'] = 'not a log level';
      const client = new OursPrivacy({ logger: logger });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'OURS_PRIVACY_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['OURS_PRIVACY_LOG'] = 'debug';
      const client = new OursPrivacy({ logger: logger, logLevel: 'off' });
  ...
  test('explicit global fetch', async () => {
    ...
    const client = new OursPrivacy({ baseURL: 'http://localhost:5000/', fetch: defaultFetch });
  });
  ...
  test('normalized method', async () => {
    ...
    const client = new OursPrivacy({ baseURL: 'http://localhost:5000/', fetch: testFetch });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new OursPrivacy({ baseURL: 'http://localhost:5000/custom/path/' });
    ...
    afterEach(() => {
      process.env['OURS_PRIVACY_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new OursPrivacy({ baseURL: 'https://example.com' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['OURS_PRIVACY_BASE_URL'] = 'https://example.com/from_env';
      const client = new OursPrivacy({});
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['OURS_PRIVACY_BASE_URL'] = ''; // empty
      const client = new OursPrivacy({});
      expect(client.baseURL).toEqual('https://api.oursprivacy.com/api/v1');
    });

    test('blank env variable', () => {
      process.env['OURS_PRIVACY_BASE_URL'] = '  '; // blank
      const client = new OursPrivacy({});
      expect(client.baseURL).toEqual('https://api.oursprivacy.com/api/v1');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new OursPrivacy({ baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['OURS_PRIVACY_BASE_URL'] = 'http://localhost:5000/env';
      const client = new OursPrivacy({});
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new OursPrivacy({ maxRetries: 4 });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new OursPrivacy({});
    expect(client2.maxRetries).toEqual(2);
  });

  describe('withOptions', () => {
    test('creates a new client with overridden options', async () => {
      const client = new OursPrivacy({ baseURL: 'http://localhost:5000/', maxRetries: 3 });
    ...
    test('respects runtime property changes when creating new client', () => {
      const client = new OursPrivacy({ baseURL: 'http://localhost:5000/', timeout: 1000 });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new OursPrivacy({ timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new OursPrivacy({ fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new OursPrivacy({ fetch: testFetch });
